...
int ptc_open(const char *title, int width, int height) {
  ...
  ptc_presskeyON   = ptc_do_nothing;
  ptc_keyRelease = ptc_do_nothing;
...
void ptc_set_on_keypress  ( void (*onkeypress)  (KeySym) ) {
  printf("Call onekeypress\n");
  ptc_presskeyON = onkeypress;
}
void ptc_set_on_keyrelease( void (*onkeyrelease)(KeySym) ) {
  printf("Call onekeyrelease\n");
  ptc_keyRelease = onkeyrelease;
}
